package com.project.bbibbi.domain.showroom.feedReplyLike.repository;
...
public interface FeedReplyLikeRepository extends JpaRepository<FeedReplyLike, Long> {
    ...
    @Query(value = "DELETE FROM feed_reply_like WHERE feed_reply_id = :feedReplyId AND member_id = :memberId",
            nativeQuery = true)
    void deleteByFeedReplyIdAndMemberId (@Param("feedReplyId") Long feedReplyId, @Param("memberId") Long memberId);
